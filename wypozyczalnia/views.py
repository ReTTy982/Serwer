...
def test(request: HttpRequest):
    ...
    response = JsonResponse(
        Publisher.objects.values().get(publisher_name="Kawka"))
    print(type(params))
    return response
...
def populate(request):
    ...
    with open('data/publisher.csv', newline='') as c:
        ...
        for row in reader:
            ...
            Publisher.objects.create(publisher_name=row[0],
                                     phone_number=row[1])
    ...
    with open('data/book.csv', newline='') as c:
        ...
        for row in reader:
            ...
            publisher = Publisher.objects.get(
                publisher_name=random.choice(publishers)["publisher_name"])

            author = Author.objects.get(id=random.choice(authors)["id"])

            Book.objects.create(book_title=row[0],
    ...
    with open('data/user.csv', newline='') as c:
        ...
        for row in reader:
            ...
            LibraryUser.objects.create(first_name=row[0],
                                       last_name=row[1],
                                       card_number=card,
                                       login=row[2],
                                       password=row[3],
                                       address=row[4],
                                       phone_number=row[5])
            user = User.objects.create_user(username=row[2], password=row[3])
            user.save()
    ...
    with open('data/librarian.csv', newline='') as c:
        ...
        for row in reader:
            branch = Branch.objects.get(id=random.choice(branches)["id"])
            Librarian.objects.create(login=row[0],
                                     password=row[1],
                                     branch=branch,
                                     first_name=row[2],
                                     last_name=row[3])
    ...
    for i in range(5000):
        ...
        BookIssue.objects.create(copy=good_copy,
                                 branch=branch,
                                 library_user=user,
                                 date_issue=start_time.date(),
                                 date_due=end_time.date(),
                                 librarian=librarian)
    ...
    if request.method == 'POST':
        print(
            BookCopy.objects.filter(
                branch=request.POST.get('branch')).values())
        return HttpResponse(
            BookCopy.objects.filter(
                branch=request.POST.get('branch')).values())
    else:
...
def search_my_books(request):
    if request.user.is_authenticated:
        ...
        for i in range(len(borrowed_books)):
            ...
            response["book_title"].append(
                borrowed_books[i].copy.book.book_title)
...
def author(request):
    if request.method == 'GET':
        param = request.query_params.__getitem__('author_name')
...
def book_copies(request):
    if request.method == 'POST':
        if request.data['delete'] == '1':
            try:
                params = request.data
                copy = BookCopy.objects.get(id=params['id'])
        ...
        try:
            ...
            for key, value in params.items():
                if key == 'author_name':
                    q_object.add(Q(book__author__author_name__contains=value),
                                 Q.AND)
                else:
...
def library_user(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            card_serializer = LibraryUserSerializer(users,
                                                    many=True,
                                                    fields=('card_number', ))
            numbers = []
            for i in range(10000, 99999):
